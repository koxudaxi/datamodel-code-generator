...
#   filename:  object_complex_enum_field_validators.json
...
class ObjectComplexEnumFieldValidators(BaseModel):
    ...
    def validate_json_schema_constraints(self):
        ...
        if 'settings' in provided_keys:
            ...
            if not (
                ...
                or (
                    json_schema_unique_key(settings_value)
                    == json_schema_unique_key({'enabled': False, 'limit': 0})
                )
            ):
