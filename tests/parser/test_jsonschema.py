...
def test_schema_id_in_extra_template_data() -> None:
    """Test that $id is exposed as schema_id in extra template data."""
    parser = JsonSchemaParser(
        json.dumps({
            "$schema": "http://json-schema.org/draft-07/schema#",
    ...
    assert model.extra_template_data.get("schema_id") == "https://example.com/schemas/user"


def test_schema_id_in_nested_definitions() -> None:
    """Test that $id is exposed for nested schema definitions."""
    parser = JsonSchemaParser(
        json.dumps({
            "$schema": "http://json-schema.org/draft-07/schema#",
            "type": "object",
            "properties": {"name": {"type": "string"}},
            "definitions": {
                "Address": {"$id": "#address", "type": "object", "properties": {"street": {"type": "string"}}}
            },
        })
    )
    parser.parse()
    ...
    address_model = next((m for m in parser.results if m.name == "Address"), None)
    assert address_model is not None
    assert address_model.extra_template_data.get("schema_id") == "#address"


def test_path_in_render_context() -> None:
    """Test that path is available in render context."""
    parser = JsonSchemaParser(
        json.dumps({
            "$schema": "http://json-schema.org/draft-07/schema#",
            "type": "object",
    ...
    assert len(parser.results) == 1
    model = parser.results[0]
    # The path should be available via model.path
    assert model.path == "#"
