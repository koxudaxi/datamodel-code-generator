...
def test_main_jsonschema_additional_properties_value_constraints_annotated(output_file: Path) -> None:
    """Test additionalProperties value constraints survive in annotated output (msgspec).

    A constrained dict value must keep its constraint the same way a constrained
    list item does; previously only list items were promoted, so dict values
    silently dropped ``minimum``/``maximum`` etc. in field-constraint modes.
    """
    expected = (
        "# generated by datamodel-codegen:\n"
        "#   filename:  output.json\n\n"
        ...
        "IntMap: TypeAlias = Annotated[int, Meta(ge=1, le=9)]\n\n\n"
        "class Payload(Struct):\n"
        "    int_map: dict[str, IntMap] | UnsetType = UNSET\n"
    )
    _run_jsonschema_dict(
        {
            "title": "Payload",
