...
class TestSchemaVersionDetection:
    """Tests for schema version detection functions."""

    def test_detect_jsonschema_version_draft04(self) -> None:
        """Test detection of JSON Schema draft-04."""
        from datamodel_code_generator import JsonSchemaVersion, detect_jsonschema_version

        data = {"$schema": "http://json-schema.org/draft-04/schema#"}
        assert detect_jsonschema_version(data) == JsonSchemaVersion.Draft04

    def test_detect_jsonschema_version_draft07(self) -> None:
        """Test detection of JSON Schema draft-07."""
        from datamodel_code_generator import JsonSchemaVersion, detect_jsonschema_version

        data = {"$schema": "http://json-schema.org/draft-07/schema#"}
        assert detect_jsonschema_version(data) == JsonSchemaVersion.Draft07

    def test_detect_jsonschema_version_2019_09(self) -> None:
        """Test detection of JSON Schema 2019-09."""
        from datamodel_code_generator import JsonSchemaVersion, detect_jsonschema_version
    ...
    def test_detect_jsonschema_version_2020_12(self) -> None:
        """Test detection of JSON Schema 2020-12."""
        from datamodel_code_generator import JsonSchemaVersion, detect_jsonschema_version

        data = {"$schema": "https://json-schema.org/draft/2020-12/schema"}
        assert detect_jsonschema_version(data) == JsonSchemaVersion.Draft202012

    def test_detect_jsonschema_version_no_schema(self) -> None:
        """Test detection with missing $schema defaults to Auto."""
        from datamodel_code_generator import JsonSchemaVersion, detect_jsonschema_version

        data = {"type": "object"}
        assert detect_jsonschema_version(data) == JsonSchemaVersion.Auto

    def test_detect_jsonschema_version_non_string_schema(self) -> None:
        """Test detection with non-string $schema defaults to Auto."""
        from datamodel_code_generator import JsonSchemaVersion, detect_jsonschema_version

        data = {"$schema": 123}
        assert detect_jsonschema_version(data) == JsonSchemaVersion.Auto

    def test_detect_openapi_version_swagger(self) -> None:
        """Test detection of OpenAPI 2.0 (Swagger)."""
        from datamodel_code_generator import OpenAPIVersion, detect_openapi_version

        data = {"swagger": "2.0"}
        assert detect_openapi_version(data) == OpenAPIVersion.V20

    def test_detect_openapi_version_30(self) -> None:
    ...
    def test_detect_openapi_version_31(self) -> None:
    ...
    def test_detect_openapi_version_no_version(self) -> None:
        """Test detection with missing version field defaults to Auto."""
        from datamodel_code_generator import OpenAPIVersion, detect_openapi_version

        data = {"paths": {}}
        assert detect_openapi_version(data) == OpenAPIVersion.Auto

    def test_detect_openapi_version_non_string(self) -> None:
        """Test detection with non-string openapi defaults to Auto."""
        from datamodel_code_generator import OpenAPIVersion, detect_openapi_version
...
class TestSchemaVersionExceptions:
    """Tests for schema version exception classes."""

    def test_unsupported_version_error(self) -> None:
        """Test UnsupportedVersionError exception."""
        from datamodel_code_generator import UnsupportedVersionError

        error = UnsupportedVersionError("1.0", ["2.0", "3.0", "3.1"])
        assert error.version == "1.0"
        assert error.supported == ["2.0", "3.0", "3.1"]
    ...
    def test_schema_validation_error(self) -> None:
        ...
        from datamodel_code_generator import SchemaValidationError

        error = SchemaValidationError("nullable not allowed", "3.1", "nullable")
        assert error.version == "3.1"
        assert error.feature == "nullable"
        assert "[3.1]" in str(error)
        assert "nullable not allowed" in str(error)
