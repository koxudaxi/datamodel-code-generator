...
DISJOINT_ALLOF_EXPECTED = EXPECTED_JSON_SCHEMA_PATH / "disjoint_allof_types"


DISJOINT_ALLOF_CASES = json.loads((DISJOINT_ALLOF_EXPECTED / "cases.json").read_text(encoding="utf-8"))
...
def test_disjoint_allof_diagnostics(
    ...
    assert_output(
        json.dumps(
            [{"payload": p, "native": validator.is_valid(p)} for p in DISJOINT_ALLOF_CASES[case]["payloads"]], indent=2
        )
        + "\n",
...
def test_compatible_allof_types(
    ...
    source = DATA_PATH / "jsonschema" / DISJOINT_ALLOF_CASES[case]["source"]
    schema = json.loads(source.read_text(encoding="utf-8"))
    validator_class = validator_for(schema)
    validator_class.check_schema(schema)
    validator = validator_class(schema)
    ...
    with _generated_model(output_file, "compatible_allof", DISJOINT_ALLOF_CASES[case]["model"]) as model:
        for payload in DISJOINT_ALLOF_CASES[case]["payloads"]:
            with assert_inputs_not_mutated({"payload": payload}):
                try:
                    model.model_validate(payload)
                    accepted = True
    ...
    assert_output(json.dumps(records, indent=2) + "\n", DISJOINT_ALLOF_EXPECTED / f"{case}_runtime.txt")
...
def test_allof_external_mappings_preserve_imports(
    ...
    with _generated_model(output_file, "mapped_allof", "MappedModel") as model:
        for payload in json.loads((DISJOINT_ALLOF_EXPECTED / "mapped_payloads.json").read_text(encoding="utf-8")):
            try:
                model.model_validate(payload)
                accepted = True
            except ValidationError:
    ...
    assert_output(json.dumps(records, indent=2) + "\n", DISJOINT_ALLOF_EXPECTED / "mapped_runtime.txt")
