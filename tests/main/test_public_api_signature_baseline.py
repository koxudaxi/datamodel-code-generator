...
def _kwonly_args(signature: inspect.Signature) -> list[str]:
    return [
        name
...
def test_generate_signature_matches_baseline() -> None:
    ...
    assert _kwonly_args(signature) == EXPECTED_GENERATE_KWONLY


def test_parser_signature_matches_baseline() -> None:
    signature = inspect.signature(Parser.__init__)
    assert _kwonly_args(signature) == EXPECTED_PARSER_KWONLY
