...
from datamodel_code_generator.config import GenerateConfigDict, ParserConfigDict
...
from datamodel_code_generator.format import DEFAULT_FORMATTERS, PythonVersionMin
from datamodel_code_generator.model.pydantic import BaseModel, CustomRootType, DataModelField, DataTypeManager
...
if TYPE_CHECKING:
    ...
    from datamodel_code_generator.parser.base import YamlValue
...
class _BaselineParser:
    def __init__(
        ...
        *,
        ...
        data_model_root_type: type[DataModel] = CustomRootType,
...
def test_generate_signature_matches_baseline() -> None:
    """Ensure GenerateConfigDict preserves origin/main kw-only args and annotations."""
    expected = _kwonly_by_name(inspect.signature(_baseline_generate))
    actual = GenerateConfigDict.__annotations__
    assert set(actual.keys()) == set(expected.keys())
    for name, param in expected.items():
        expected_type = str(param.annotation)
        actual_type = str(actual[name])
        if actual_type.startswith("NotRequired[") and actual_type.endswith("]"):
            actual_type = actual_type[len("NotRequired[") : -1]
        assert actual_type == expected_type


def test_parser_signature_matches_baseline() -> None:
    """Ensure ParserConfigDict preserves origin/main kw-only args and annotations."""
    expected = _kwonly_by_name(inspect.signature(_BaselineParser.__init__))
    actual = ParserConfigDict.__annotations__
    assert set(actual.keys()) == set(expected.keys())
    for name, param in expected.items():
        expected_type = str(param.annotation)
        actual_type = str(actual[name])
