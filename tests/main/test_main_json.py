...
def test_main_json_array_include_null() -> None:
    with TemporaryDirectory() as output_dir:
        ...
        assert return_code == Exit.OK
        assert (
            output_file.read_text(encoding="utf-8") == (EXPECTED_JSON_PATH / "json_array_include_null.py").read_text()
        )
...
def test_main_json_reuse_model_pydantic2() -> None:
    with TemporaryDirectory() as output_dir:
        ...
        assert (
            output_file.read_text(encoding="utf-8")
            == (EXPECTED_JSON_PATH / "json_reuse_model_pydantic2.py").read_text()
        )
