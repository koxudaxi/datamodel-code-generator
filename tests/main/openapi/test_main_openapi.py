...
def test_generate_openapi_keeps_referenced_path_item_original_unchanged(tmp_path: Path) -> None:
    """Keep cached referenced OpenAPI path items unchanged while inheriting operation metadata."""
    input_path = tmp_path / "openapi.yaml"
    path_item_path = tmp_path / "path-item.yaml"
    output_path = tmp_path / "output.py"
...
paths:
  /cached:
    $ref: ./path-item.yaml#/paths/~1items
components:
  schemas:
    Item:
      type: object
