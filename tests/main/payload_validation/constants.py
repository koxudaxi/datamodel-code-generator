...
_SCHEMA_FILE_SUFFIXES = {".json", ".yaml", ".yml"}
_COMBINED_SCHEMA_KEYS = ("allOf", "anyOf", "oneOf")
_JSON_SCHEMA_KEYS = {
    "$defs",
    "$id",
    "$ref",
...
_SIMPLE_ALLOF_IGNORED_KEYS = {"description", "title"}
...
def _payload_target_python_version() -> str:
    if is_supported_in_black(_CURRENT_PYTHON_VERSION):
        return CURRENT_PYTHON_VERSION
    supported_versions = [
        python_version
        for python_version in PythonVersion
        if python_version.version_key <= _CURRENT_PYTHON_VERSION.version_key and is_supported_in_black(python_version)
    ]
    return max(supported_versions, key=lambda python_version: python_version.version_key).value


_PAYLOAD_TARGET_PYTHON_VERSION = _payload_target_python_version()
