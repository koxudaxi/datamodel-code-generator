...
def test_format_code_un_exist_custom_formatter():
    with pytest.raises(ModuleNotFoundError):
        _ = CodeFormatter(
            PythonVersion.PY_37, custom_formatters=[UN_EXIST_FORMATTER],
        )
...
def test_format_code_invalid_formatter_name():
    with pytest.raises(NameError):
        _ = CodeFormatter(
            PythonVersion.PY_37, custom_formatters=[WRONG_FORMATTER],
        )
...
def test_format_code_is_not_subclass():
    with pytest.raises(TypeError):
        _ = CodeFormatter(
            PythonVersion.PY_37, custom_formatters=[NOT_SUBCLASS_FORMATTER],
        )


def test_format_code_with_custom_formatter_without_kwargs():
    formatter = CodeFormatter(
        PythonVersion.PY_37, custom_formatters=[ADD_COMMENT_FORMATTER],
    )
...
def test_format_code_with_custom_formatter_with_kwargs():
    formatter = CodeFormatter(
        PythonVersion.PY_37, custom_formatters=[ADD_LICENSE_FORMATTER],
        custom_formatters_kwargs={"license_file": EXAMPLE_LICENSE_FILE}
    )

    formatted_code = formatter.format_code("x = 1\ny = 2")

    assert formatted_code == """# MIT License
# 
