...
class JsonSchemaParser(Parser["JSONSchemaParserConfig", "JsonSchemaFeatures"]):
    ...
    @classmethod
    def _drop_null_type_when_allof_branches_exclude_null(cls, schema_dict: dict[Any, Any]) -> None:
        type_values = cls._type_values(schema_dict.get("type"))
        if type_values is None or "null" not in type_values:
            return
        if not type_values <= {"integer", "number", "string", "null"}:
            return
    ...
    @classmethod
    def _raw_null_matches_supported_schema(cls, schema_dict: Any) -> bool | None:  # noqa: PLR0911, PLR0912
        ...
        if set(schema_dict) - supported_keys:
            return None
        if not cls._raw_value_matches_schema(None, schema_dict):
            return False
    ...
    def _normalize_raw_allof_count_combined_constraints(cls, schema_dict: dict[Any, Any]) -> None:  # noqa: PLR0911, PLR0912
        ...
        for item in all_of:
            ...
            if item is False:
                return
            if not isinstance(item, dict) or not cls._raw_allof_count_combined_item_is_mergeable(item):
                return
        ...
        cls._normalize_raw_count_combined_constraints(merged_schema)
        if merged_schema == before:
            return
    ...
    @classmethod
    def _raw_allof_count_combined_item_is_mergeable(cls, schema_dict: dict[Any, Any]) -> bool:
        unsupported_keys = {
            "$ref",
            "additionalProperties",
            "allOf",
    ...
    def _validate_raw_numeric_constraints(cls, schema_dict: dict[Any, Any]) -> None:
        ...
        cls._validate_integer_bounds_have_candidate(schema_dict)
        cls._validate_numeric_multiple_of_bounds(schema_dict)

    @classmethod
    def _validate_integer_bounds_have_candidate(cls, schema_dict: dict[Any, Any]) -> None:
        type_values = cls._type_values(schema_dict.get("type"))
        if type_values is None or cls._simplify_type_values(type_values) != {"integer"}:
            return
        ...
        lower_value, lower_exclusive = lower_bound
        upper_value, upper_exclusive = upper_bound
        min_integer = cls._ceil_fraction(lower_value)
        if lower_exclusive and min_integer == lower_value:
            min_integer += 1

        max_integer = cls._floor_fraction(upper_value)
    ...
    @classmethod
    def _validate_raw_string_constraints(cls, schema_dict: dict[Any, Any]) -> None:
        cls._validate_allof_count_bounds(schema_dict, "minLength", "maxLength")
