...
class JsonSchemaParser(Parser["JSONSchemaParserConfig", "JsonSchemaFeatures"]):
    ...
    def get_data_type(self, obj: JsonSchemaObject) -> DataType:
        ...
        if isinstance(obj.type, list):
            ...
            return (
                self.data_type(
                    data_types=[_get_data_type(t, obj.format or "default") for t in non_null_types],
    ...
    def _field_original_name_mapping(self, fields: list[DataModelFieldBase]) -> dict[str, str]:  # noqa: PLR6301
        return {
            field.name: field.original_name
            for field in fields
            if field.name and field.original_name and field.original_name != field.name
        }
    ...
    def _schema_needs_raw_value_validator(
        self,
        schema: JsonSchemaObject | bool,  # noqa: FBT001
        ...
        if schema.ref:
            if schema.ref in visited_refs:
                return False
            return self._schema_needs_raw_value_validator(
                self._load_ref_schema_object(schema.ref),
                visited_refs | {schema.ref},
            )
        raw_value_markers = (
            ...
            schema.propertyNames is not None,
            schema.minProperties is not None,
            schema.maxProperties is not None,
            schema.additionalProperties is False,
            isinstance(schema.additionalProperties, JsonSchemaObject),
            schema.unevaluatedProperties is False,
            isinstance(schema.unevaluatedProperties, JsonSchemaObject),
        )
        if any(raw_value_markers):
            return True
        child_schemas: list[JsonSchemaObject | bool] = [*schema.anyOf, *schema.oneOf, *schema.allOf]
        ...
        child_schemas.extend(
            dependent_schema for _, dependent_schema in self._iter_dependent_schema_objects(schema.extras)
        )
    ...
    def _set_root_raw_value_model_validators(self, path: str, obj: JsonSchemaObject) -> None:
        if not self._supports_root_json_schema_validators() or not self._root_raw_value_needs_runtime_validator(obj):
            return
        predicate = self._schema_runtime_value_predicate(obj, "root_value")
        if not predicate or predicate == "True":
            return
        ...
        validators = self.extra_template_data[path].setdefault("json_schema_validators", {})
        validators["root_raw_value"] = {"predicate": predicate}
    ...
    def _collect_dependent_schema_property_validators(
        ...
        for property_name, dependent_schema in self._iter_dependent_schema_objects(extras):
            ...
            condition = self._schema_condition_predicate(dependent_schema)
            if condition:
    ...
    def _collect_not_validators(
        ...
        not_schema = self.SCHEMA_OBJECT_TYPE.model_validate(raw_not_schema)
        condition = self._schema_condition_predicate(not_schema)
    ...
    def _set_object_model_validators(
        ...
        validators: dict[str, Any] = {}
        validators["field_original_names"] = self._field_original_name_mapping(fields)
        ...
        conditional_validators = self._collect_conditional_validators(obj.extras)
        if conditional_validators:
            validators["conditional"] = conditional_validators

        not_validators = self._collect_not_validators(obj.extras)
        if not_validators:
