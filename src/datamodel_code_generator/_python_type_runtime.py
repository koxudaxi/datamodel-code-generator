...
def _callable_expr_from_runtime_args(args: tuple[object, ...], *, qualify: bool) -> PythonTypeExpr:
    ...
    if len(args) == 2:  # noqa: PLR2004
        ...
        match parameters:
            case _ if parameters is ...:
                parameter_expression: PythonTypeExpr = PythonTypeEllipsis()
            case list() | tuple():
                parameter_expression = PythonTypeParameterList(
                    tuple(_python_type_expr_from_runtime(item, qualify=qualify) for item in parameters)
                )
            case _ if isinstance(parameters, ParamSpec) or get_origin(parameters) is Concatenate:
                parameter_expression = _python_type_expr_from_runtime(parameters, qualify=qualify)
            case _:
                parameter_expression = PythonTypeParameterList(
                    tuple(_python_type_expr_from_runtime(item, qualify=qualify) for item in args[:-1])
                )
        return PythonTypeSubscript(
            python_type_name("Callable"),
            (parameter_expression, _python_type_expr_from_runtime(return_type, qualify=qualify)),
        )
    *parameters, return_type = args
    return PythonTypeSubscript(
        python_type_name("Callable"),
