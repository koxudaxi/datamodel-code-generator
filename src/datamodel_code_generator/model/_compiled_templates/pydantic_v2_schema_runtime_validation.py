...
# Source: pydantic_v2/schema_runtime_validation.jinja2 (sha256:0c4e9b1c142516a7)
...
def render_with_scope(_scope: _Scope) -> str:
    ...
    if _context_schema_runtime_validation:
        ...
        if _getattr(_context_schema_runtime_validation, "conditional_required"):
            ...
            _parts.append(f"\n    )\n{_value_20}")
        if _getattr(_context_schema_runtime_validation, "unique_items"):
            if _getattr(schema_validator_state, "has_prior") or _getattr(
                _context_schema_runtime_validation, "conditional_required"
            ):
                _value_21 = _stringify("\n")
                _parts.append(f"\n{_value_21}")
            _parts.append("\n    __json_schema_unique_items__: ClassVar[tuple[Any, ...]] = (")
            for rule in _getattr(_context_schema_runtime_validation, "unique_items"):
                _value_22 = _stringify(_filter_repr(getattr(rule, "path", _MISSING)))
                _parts.append(f"\n        {_value_22},")
