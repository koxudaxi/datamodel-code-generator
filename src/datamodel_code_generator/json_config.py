...
def _invalid_json_message(option_name: str, error: json.JSONDecodeError, load_error_name: str | None) -> str:
    if load_error_name is not None:
...
def _load_json_source(value: JsonConfigSource, *, option_name: str, load_error_name: str | None = None) -> Any:
    ...
    path = Path(value_as_string).expanduser()
    try:
    ...
    if is_file:
        try:
            ...
            msg = f"Unable to read JSON file {value_as_string!r}: {e}"
            raise JsonConfigError(msg) from e
    else:
        json_input = value_as_string
...
class JsonConfigSpec:
    ...
    def validate(self, raw: Any, *, warn_on_legacy_fallback: bool = True) -> Any:
        ...
        if self.as_defaultdict:
            return _to_defaultdict(result)
        return result
    ...
    def _validate_legacy(
        ...
        *,
        ...
        if self.legacy_model is None:
            ...
            if self.validation_error_name is not None:
                msg = f"Invalid {self.validation_error_name}: {error_message}"
            elif self.load_error_name is not None:
                msg = f"Unable to load {self.load_error_name}: {error_message}"
            else:
                msg = f"Invalid {self.option_name}: {error_message}"
            raise JsonConfigError(msg) from strict_error
