...
def alias_bound_python_type(
    ...
    def alias_leaf(expression: PythonTypeExpr) -> PythonTypeExpr:
        match expression:
            case PythonTypeBoundName(import_from=from_, import_name=import_name) if aliased_import := aliases.get((
                from_,
                import_name,
            )):
            ...
            case PythonTypeRuntimeSymbol(module=module) if alias := module_aliases.get(module):
                return PythonTypeRuntimeSymbol(alias, expression.qualname_parts)
        return expression
