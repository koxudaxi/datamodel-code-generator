...
if TYPE_CHECKING:
    from datamodel_code_generator._types.generate_config_dict import (
    ...
    **options: Any,
    ...
    Args:
        ...
        config: A GenerateConfig object containing all generation options.
            Cannot be used together with individual options.
        options: Individual generation options (see GenerateConfig for details).
            Cannot be used together with config parameter.
    ...
    Raises:
        ValueError: If both config and individual options are provided.
    """
    # Exclusivity check: config and options cannot be used together
    if config is not None and options:
        msg = "Cannot specify both 'config' and individual options"
        raise ValueError(msg)

    # Create GenerateConfig from options or use default
    if options:
        from datamodel_code_generator.config import GenerateConfig as _GenerateConfig  # noqa: PLC0415

        config = _GenerateConfig(**options)
    elif config is None:
        from datamodel_code_generator.config import GenerateConfig as _GenerateConfig  # noqa: PLC0415
    ...
    extra_template_data: defaultdict[str, dict[str, Any]] | None = (
        defaultdict(dict, config.extra_template_data) if config.extra_template_data else None
    )
    ...
    if dataclass_arguments is None:
        dataclass_arguments = cast("DataclassArguments", {})
