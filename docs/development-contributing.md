...
Normal test environments use the Ruff version recorded in `uv.lock`. The Ruff
formatter integration tests copy
`tests/data/config/pyproject_ruff_formatter_contract.toml` into the parent of
each generated output. Its `E4`, `E7`, `E9`, and `F` selection is Ruff's legacy
default rule set, so the tests hold generated bytes, batch formatting, and
runtime imports stable without inheriting new Ruff default rules.

Run the locked focused CLI tests with the normal Python test environment:

```bash
tox run -e py314-nocov-parallel -- \
  tests/main/test_main_general.py::test_ruff_check_and_format_combined \
  tests/main/test_main_general.py::test_ruff_check_only \
  tests/main/test_main_general.py::test_ruff_format_only \
  tests/main/test_main_general.py::test_ruff_batch_formatting_directory \
  tests/main/test_main_general.py::test_type_checking_imports_default_to_runtime_imports_for_modular_pydantic_ruff \
  tests/main/test_main_general.py::test_no_use_type_checking_imports
```

Run the same focused contract against a newer Ruff release with:

```bash
tox run -e ruff-latest
```

That lane starts with the locked environment and upgrades only Ruff before its
six formatter tests. The pre-commit `ruff` hook is separate: it lints this
repository's source with the root `pyproject.toml` policy and does not define
the generated-output formatter contract.

The explicit rule selection avoids changes from Ruff's evolving defaults, but
an upstream Ruff change can still affect generated output. Review the upstream
change and exact generated diff before updating expectations in a separate,
reviewed change.
