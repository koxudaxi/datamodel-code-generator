...
|----|------|--------|---------------|---------|-------------|
| `format.default-formatters` | behavior | `Default formatters` | 0.52.0 | 1.0.0 | Set formatters explicitly, for example ruff-format and ruff-check. |
| `behavior.pydantic-v2-use-annotated-default` | behavior | `Pydantic v2 default for --use-annotated` | 0.52.1 | 1.0.0 | Explicitly pass --use-annotated or --no-use-annotated. |
| `behavior.remote-ref-default` | behavior | `Remote $ref fetching without --allow-remote-refs` | 0.56.0 | 1.0.0 | Pass --allow-remote-refs or --no-allow-remote-refs explicitly. |
| `cli.allow-extra-fields` | cli-option | `--allow-extra-fields` | 0.31.0 | 1.0.0 | --extra-fields=allow |
| `cli.parent-scoped-naming` | cli-option | `--parent-scoped-naming` | 0.48.0 | 1.0.0 | --naming-strategy parent-prefixed |
| `cli.validation` | cli-option | `--validation` | 0.24.0 | 1.0.0 | Use a dedicated OpenAPI validation tool. |
| `config.yaml-non-lowercase-bool` | config | `YAML bool values True, False, TRUE, FALSE` | 0.48.0 | 1.0.0 | Use lowercase true or false. |
| `python-api.python-version-has-type-alias` | python-api | `PythonVersion.has_type_alias` | 0.52.1 | 1.0.0 | - |
| `schema.jsonschema-items-array` | schema | `JSON Schema Draft 2020-12 items array tuple validation` | 0.53.0 | 1.0.0 | Use prefixItems. |
| `schema.openapi-nullable` | schema | `OpenAPI 3.1 nullable keyword` | 0.53.0 | 1.0.0 | Use type arrays such as type: ["string", "null"]. |
